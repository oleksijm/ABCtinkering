...
//started on 13.08.2023.
// update - 14.08.2023 - added 'remove deprecation warning' and set to C++20 - WORKS
// last update - 15.08.2023 - adding float variables to accommodate floating point numbers (double)
// should work with max number of Excel rows. For larger numbers the program needs to be rewritten to
...
WCHAR s_text_1[]{ L"Random Text.." }; //actual maximum number of characters in box depends on box window (hwnd) length!
int posX = 0;
int posY = 0;
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		hTxtInput = CreateWindowEx(WS_EX_CLIENTEDGE, TEXT("EDIT"), s_text_1,
			WS_VISIBLE | WS_CHILD | ES_LEFT, 50, 50, 510, 25, hWnd, (HMENU)IDC_TEXTBOX, hMainInstance, NULL);

		hButton = CreateWindowEx(WS_EX_CLIENTEDGE, TEXT("BUTTON"), TEXT("Press me!"), WS_VISIBLE | WS_CHILD | WM_COPY | ES_LEFT,
			600, 50, 400, 25, hWnd, (HMENU)IDC_BUTTON, hMainInstance, NULL);
